...
def predict(query, trainer, pre_encoded=None):
    start_with_bos = '<|endoftext|>' + query
    encoded_prompt = trainer.tokenizer(start_with_bos, add_special_tokens=False, return_tensors="pt").input_ids
    if pre_encoded:
        encoded_prompt = torch.cat((pre_encoded, encoded_prompt))
    ...
    for i, g in enumerate(generated_sequences):
        predictions.append([query, g])
    return predictions, encoded_prompt

def get_encoding(reply, trainer):
    start_with_bos = '<|endoftext|>' + reply
    encoded_prompt = trainer.tokenizer(start_with_bos, add_special_tokens=False, return_tensors="pt").input_ids
    encoded_prompt = encoded_prompt.to(trainer.model.device)
    return encoded_prompt 

def start(handle, chat_history = False):
    HANDLE = handle 
    get_data(handle)
    trainer = finetune(handle)
    TRAINER = trainer 

    if chat_history: 
        PRE_ENCODED = torch.load(chat_history)
        return 
    init_prompt = f"""You are {handle}. A conversation with me looks like this:  
    Me: says something
    {handle}: responds
    For every Me: input you have to generate a response from {handle}:
    Me: Hi, I would like to have a conversation with you.
    {handle}: Sure, let's have a conversation. 
    Me: Can I ask you a question?
    {handle}:"""

    reply, PRE_ENCODED = predict(init_prompt, TRAINER)
    PRE_ENCODED = torch.cat((PRE_ENCODED, get_encoding(reply+"\n", TRAINER)))


def get_reply(query):
    if str.to_lower(query) == "end conversation":
        torch.save(PRE_ENCODED, "chat_history.pt")
        return 
    query = "Me:" + query + "\n" + HANDLE + ":"
    reply, PRE_ENCODED = predict(query, TRAINER, PRE_ENCODED)
    PRE_ENCODED = torch.cat((PRE_ENCODED, get_encoding(reply+"\n", TRAINER)))
    print(reply)
